        ...
        public JsonResult GetPOItemCount()
        {
            try
            {
                ...
                if (role.ToLower() == LoginConstants.RoleVendor.ToLower())
                {
                    ...
                    subcontNewPO = pOesSubcontNewPO.Where(po => vendorCode == po.VendorCode).SelectMany(x => x.PurchasingDocumentItems).Count(x => x.ConfirmedQuantity == null && !String.IsNullOrEmpty(x.Material) && x.ParentID == null && String.IsNullOrEmpty(x.IsClosed) && y.PurchasingDocumentItemHistories.Count == 0);
                ...
                else if (role.ToLower() == LoginConstants.RoleSubcontDev.ToLower())
                {
                    ...
                    subcontNewPO = pOesSubcontNewPO.Where(po => vendorSubcont.Contains(po.VendorCode)).SelectMany(x => x.PurchasingDocumentItems).Count(x => x.ConfirmedQuantity == null && !String.IsNullOrEmpty(x.Material) && x.ParentID == null && String.IsNullOrEmpty(x.IsClosed) && y.PurchasingDocumentItemHistories.Count == 0);
